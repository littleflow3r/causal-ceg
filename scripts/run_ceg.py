...
from utils_ceg import RelationLoader, MyDataLoader
from model_ceg import RE_LCKG
from evaluate import Eval

class Runner(object):
    def __init__(self, id2rel, loader, user_config, lckg_embeddings):
        self.class_num = len(id2rel)
        self.id2rel = id2rel
        self.loader = loader
        self.user_config = user_config
        self.model = RE_LCKG(self.class_num, user_config, lckg_embeddings)
...
def print_result(floc, predict_label, id2rel, start_idx=0):
    des_file = floc+'/predicted_result_lckg.txt'
    with open(des_file, 'w', encoding='utf-8') as fw:
        fw.write('{}\t{}\n'.format('index', 'prediction'))
        for i in range(0, predict_label.shape[0]):
            fw.write('{}\t{}\n'.format(
                start_idx+i, predict_label[i]))
...
if __name__ == '__main__':
    ...
    loader = MyDataLoader(rel2id, user_config)
    ...
    runner = Runner(id2rel, loader, user_config, lckg_embeddings)
    if user_config.mode == 0:  # train mode
        runner.train()
    ...
    print_result(user_config.model_dir, predict_label, id2rel)
